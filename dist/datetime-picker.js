    ...
    .constant('uiDatetimePickerConfig', {
        ...
        buttonBar: {
            ...
            today: {
                show: true,
                text: 'Today',
                cls: 'btn-sm btn-default btn-today'
            },
            clear: {
                show: true,
                text: 'Clear',
    ...
    .controller('DateTimePickerController', ['$scope', '$element', '$attrs', '$compile', '$parse', '$document', '$timeout', '$uibPosition', 'dateFilter', 'uibDateParser', 'uiDatetimePickerConfig', '$rootScope',
        function ($scope, $element, $attrs, $compile, $parse, $document, $timeout, $uibPosition, dateFilter, uibDateParser, uiDatetimePickerConfig, $rootScope) {
            ...
            this.init = function (_ngModel) {
                ...
                // Now you can control timezone bar and time planel from HTML
...
angular.module('ui.bootstrap.onedatetimepicker').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('template/date-picker.html',
    "<ul class=\"dropdown-menu dropdown-menu-left datetime-picker-dropdown\" ng-if=\"isOpen && showPicker == 'date'\" ng-style=dropdownStyle style=left:inherit ng-keydown=keydown($event) ng-click=$event.stopPropagation()><li style=\"padding:0 5px 5px 5px\" class=date-picker-menu><div ng-transclude></div></li><li style=padding:5px ng-if=buttonBar.show><span class=\"btn-group pull-left\" style=margin-right:10px ng-if=\"doShow('today') || doShow('clear')\"><button type=button class=btn ng-class=\"getClass('today')\" ng-if=\"doShow('today')\" ng-click=\"select('today', $event)\" ng-disabled=\"isDisabled('today')\">{{ getText('today') }}</button> <button type=button class=\"btn btn-sm btn-danger\" ng-if=\"doShow('clear')\" ng-click=\"select('clear')\">{{ getText('clear') }}</button></span> <span class=\"btn-group pull-right\" ng-if=\"(doShow('time') && enableTime) || doShow('close')\"><button type=button class=\"btn btn-sm btn-success\" ng-if=\"doShow('close')\" ng-click=close(true)>{{ getText('close') }}</button></span></li><li class=\"calendar-timezone-bar-li\" ng-if=timezoneBar><span class=\"calendar-timezone-bar\">Timezone: {{ timezoneBar }}</span></li></ul>"
  );
...
if(typeof exports === 'object' && typeof module === 'object') {
    module.exports = 'ui.bootstrap.onedatetimepicker';
}
